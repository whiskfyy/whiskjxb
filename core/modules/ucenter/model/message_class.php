...
class msm_ucenter_message extends ms_model {
    ...
    function send($senduid, $recvuid, $subject, $content, $isusername = FALSE, $type = 0) {
        if(!$senduid) { //公告信息
        ...
        $return = uc_pm_send($senduid, implode(',',$recvuids), $subject, $content, 1, 0, $isusername?1:0, $type);
    ...
    function check_post(& $recvuid, & $subject, & $content) {
        if(!$recvuid) {
            ...
            redirect(sprintf(lang('member_pm_strlen_subject'), 60));
        } elseif(strlen($content) > 500) {
            redirect(sprintf(lang('member_pm_strlen_content'), 500));
        }
    }

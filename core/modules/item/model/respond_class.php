...
class msm_item_respond extends msm_item_itembase {
	...
	function check_post(& $post, $isedit = FALSE) {
		...
		if(!$post['content']) redirect('item_respond_empty_content');
        $mod = $this->variable('config');
        $len = strlen($post['content']);
		if($len > $mod['respond_max'] || $len < $mod['respond_min']) {
            redirect(lang('item_respond_content_charlen', array($mod['respond_min'], $mod['respond_max'])));
        }
	}
    ...
    function _feed($respondid) {
        ...
        if(!$detail = $this->read($respondid)) return;
        if(!$detail['uid']) return;

        $feed = array();
        $feed['icon'] = lang('item_respond_feed_icon');
        $feed['title_template'] = lang('item_respond_feed_title_template');
        $feed['title_data'] = array (
            'username' => '<a href="'.url("space/index/uid/$detail[uid]").'">' . $detail['username'] . '</a>',
        );
        $feed['body_template'] = lang('item_respond_feed_body_template');

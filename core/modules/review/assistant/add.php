...
if(!$_POST['dosubmit']) {
    ...
    if (isset($subject) && strlen($subject)>=15){
        $name_len = strlen($subject);
        $name_s = substr($subject,0,6);
        $name_e = substr($subject,$name_len-3,3);
        $subject = $name_s . '*********' . $name_e;
    }
    //added by whisk end
